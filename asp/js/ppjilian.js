...
//根据地区得到省       
function GetProvince(pid,ppid) {
    ClearPOptions(); //先清空所有选项
    var ppid = ppid.toString();
    var answer;
    ...
    xmlHttp.open("get", "ppxx_ajax.aspx?type=1&pid=" + pid + "&r=" + Math.random + "&ppid=" + ppid, true);
    xmlHttp.send();
}
...
function GetCity(cid, ppid) {
    ...
    xmlHttp.open("get", "ppxx_ajax.aspx?type=2&cid=" + cid + "&r=" + Math.random + "&ppid=" + ppid, true);
    xmlHttp.send();
}
        ...
        if (str_param[1].indexOf("$") > 0)//分割供应商信息
        {
            ...
            for (var j = 0; j < str_gysxx.length; i++) {
                ...
                if (str_gysxx[j] != null && str_gysxx[j].indexOf("|") > 0) {
                    ...
                    array.push("><div class='fxs2'><ul>");
                    array.push("<li class='fxsa'>");
