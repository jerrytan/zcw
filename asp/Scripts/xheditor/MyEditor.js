...
function MyEditor(editor, op)
{
	this.editor = MyEditor.check(editor);
	this.contentLength = 8000;
	this.disabled = false;
	this.options = MyEditor.extend({}, op || {});
	this.hashCode = "CsdnUbb_" + (MyEditor.counter++).toString(36);
	MyEditor.instances[this.hashCode] = this;
	this.initialize();
};

MyEditor.defaultLanguage = function ()
{
...
MyEditor.extend = function(d, s)
{
	for (var i in s) d[i] = s[i]; 
	return d;
};

MyEditor.check = function(e)
{
...
MyEditor.loadCssFile = function(cssPath, uniqueId)
{
	if (/\w+\.\w+(\?|$)/.test(cssPath))
	{
...
MyEditor.counter = 0;
MyEditor.instances = {};
MyEditor.instance = function(i)
{
	return MyEditor.instances[i];
}

MyEditor.realOffset = function(o)
{
	var e = o, x = y = l = t = 0, doc = MyEditor.body();
...
(
	function()
	{
		...
		MyEditor.path = (t.lastIndexOf("/")<0) ? "." : t.substring(0, t.lastIndexOf("/"));
	}
)	();

MyEditor.loadCssFile(MyEditor.path + "/MyEditor.css", "MyEditor_CSS");

MyEditor.contentCounter = function(hashcode)
{
	var box = MyEditor.check("wordCounter_" + hashcode);
	if (box)
	{
		var editor = MyEditor.instance(hashcode);
...
MyEditor.disable = function(input, hashcode)
{
	var editor = MyEditor.instance(hashcode);
...
MyEditor.prototype.initialize = function()
{
	if (!this.editor) return;
	var me = this;
	this.editor.onkeyup = function() { MyEditor.contentCounter(me.hashCode); }
	this.editor.onfocus = function() { MyEditor.contentCounter(me.hashCode); }
	this.editor.className += " MyEditor";

	if (this.editor.form && isIE && !window.opera)
	{
		this.editor.form.attachEvent("onsubmit", function()
		{
			var e = MyEditor.check(me.pasteInputId);
			if (e)
			{
	...
	this.editor.insertAdjacentHTML("beforeBegin",
		"<div class='MyEditorMenubar' id='MyEditorMenubar_" + this.hashCode + "' " + (this.disabled ? " style='display: none'" : "") + "></div>");
	this.editor.insertAdjacentHTML("afterEnd",
		"<div class='MyEditorStatusbar' id='MyEditorStatusbar_" + this.hashCode + "'></div>");
	this.menubar = MyEditor.check("MyEditorMenubar_" + this.hashCode);
	this.statusbar = MyEditor.check("MyEditorStatusbar_" + this.hashCode);
...
MyEditor.prototype.codeLayer = function()
{
	var item = "<div title='\u8bed\u8a00\u79cd\u7c7b\uff1a{0}' onclick=\"MyEditor.instance('" +
		this.hashCode + "').code('{0}')\">{0}</div>";
	var div = document.createElement("DIV"), a = [], me=this;
	div.className = "MyEditorCodeLayer";
	div.style.display="none";
	div.id = "MyEditorCodeLayer_" + this.hashCode;
	div.onclick = function(e) { (window.event||e).cancelBubble = true; }
	var language = MyEditor.defaultLanguage();
	if (language != "")
	{
		var item2 = "<div title='\u9ED8\u8BA4\u8BED\u8A00{0}' onclick=\"MyEditor.instance('" +
...
MyEditor.prototype.fontsizeLayer = function()
{
	var item = "<td title='size:{0}' onclick=\"MyEditor.instance('" + this.hashCode + "').fontsize('{0}')\">{0}</td>";
	var div = document.createElement("DIV"), a = [], me = this;
	div.className = "MyEditorFontsizeLayer";
	div.style.display = "none";
	div.id = "MyEditorFontsizeLayer_" + this.hashCode;
	div.onclick = function(e) { (window.event || e).cancelBubble = true; }
...
MyEditor.prototype.colorLayer = function()
{
	var s = "<td title=\"{1}\" onclick=\"MyEditor.instance('" + this.hashCode + 
		"').color('{0}')\" style=\"background-color: {0}\">&nbsp;</td>"
	var div = document.createElement("DIV"), a = [], me = this;
	div.className = "MyEditorColorLayer";
	div.style.display = "none";
	div.id = "MyEditorColorLayer_" + this.hashCode;
	div.onclick = function(e) { (window.event || e).cancelBubble = true; }
	a.push("<table border='0' cellpadding='0' cellspacing='6'>");
	a.push("<tr>");
	...
	a.push("<input type='button' class='button' onclick=\"MyEditor.instance('" + this.hashCode + 
	...
	div.innerHTML = a;
	document.body.insertBefore(div, document.body.firstChild);
	document.attachEvent("onclick", function(){me.hideLayer("MyEditorColorLayer_" + me.hashCode);});
};

var t = "<a><input onfocus='this.blur()' type='button' title='{2}' value=' ' class='menuitem {1}' onclick='MyEditor.instance(\"{0}\").execCommand(event, \"{1}\", this)' /></a>";
MyEditor.directorHTML = 
{
	...
	help : "<a><input onfocus='this.blur()' type='button' title='help' value=' ' class='menuitem help' onclick='MyEditor.instance(\"{0}\").help()' /></a>", 
	space : "<input onfocus='this.blur()' type='button' title='' value=' ' class='menuitem space' disabled='true' />",
	increase : "<a><input onfocus='this.blur()' type='button' title='\u653e\u5927\u7f16\u8f91\u533a' value=' ' class='menuitem increase' onclick='MyEditor.instance(\"{0}\").increase()' /></a>",
	decrease : "<a><input onfocus='this.blur()' type='button' title='\u7f29\u5c0f\u7f16\u8f91\u533a' value=' ' class='menuitem decrease' onclick='MyEditor.instance(\"{0}\").decrease()' /></a>"
}
...
MyEditor.prototype.render = function(ubblist)
{
	...
	{
		var s = MyEditor.directorHTML[ubblist[i].toLowerCase()] || "";
	...
	s =	"<span>" + (isIE ? "\u60A8\u53EA\u80FD\u8F93\u5165" : "\u60A8\u8FD8\u53EF\u8F93\u5165") + " <strong id='wordCounter_" + this.hashCode + "'>" +
		this.contentLength + "</strong> \u4e2a\u5b57\u7b26</span><input type='checkbox' name='MyEditorDisabled' value='true' " +
		(this.disabled ? " checked='checked'" : "") + " onclick='MyEditor.disable(this, \"" +
...
MyEditor.prototype.wraptag = function(open, close)
{
	this.editor.focus();
...
MyEditor.prototype.execCommand = function(e, ubbtag, trigger)
{
	(window.event || e).cancelBubble=true;
	var e = MyEditor.director[ubbtag];
...
MyEditor.prototype.increase = function()
{
...
MyEditor.prototype.decrease = function()
{
...
MyEditor.prototype.help = function()
{
	if (this.helpLink) window.open(this.helpLink, "_blank");
	//if you want modify the help function ,write you function at here //2008-07-20 by iux
}

MyEditor.prototype.colorlayer = function(button)
{
	var xy = MyEditor.realOffset(button);
	var layer = MyEditor.check("MyEditorColorLayer_" + this.hashCode);
	layer.style.top = (xy.y + button.offsetHeight) + "px";
	layer.style.left = xy.x + "px";
	this.showLayer(layer);
}

MyEditor.prototype.showColorLayer = function()
{
	var layer = MyEditor.check("MyEditorColorLayer_" + this.hashCode);
...
MyEditor.prototype.hideColorLayer = function()
{
	var layer = MyEditor.check("MyEditorColorLayer_" + this.hashCode);
	if (layer.style.display == "none") return;
...
MyEditor.prototype.color = function(color)
{
	this.hideLayer("MyEditorColorLayer_" + this.hashCode);
	var rule = MyEditor.director["color"];
	this.wraptag(rule.open.format(color), rule.close);
}

MyEditor.prototype.customColor = function(button)
{
...
MyEditor.prototype.codelayer = function(button)
{
	var xy = MyEditor.realOffset(button);
	var layer = MyEditor.check("MyEditorCodeLayer_" + this.hashCode);
	layer.style.top = (xy.y + button.offsetHeight) + "px";
	layer.style.left = xy.x + "px";
	this.showLayer(layer);
};

MyEditor.prototype.code = function(lang)
{
	this.hideLayer("MyEditorCodeLayer_" + this.hashCode);
	var rule = MyEditor.director["code"];
	this.wraptag(rule.open.format(lang), rule.close);
};

MyEditor.prototype.fontsizelayer = function(button)
{
	var xy = MyEditor.realOffset(button);
	var layer = MyEditor.check("MyEditorFontsizeLayer_" + this.hashCode);
	layer.style.top = (xy.y + button.offsetHeight) + "px";
	layer.style.left = xy.x + "px";
	this.showLayer(layer);
};

MyEditor.prototype.fontsize = function(size)
{
	this.hideLayer("MyEditorFontsizeLayer_" + this.hashCode);
	var rule = MyEditor.director["fontsize"];
	this.wraptag(rule.open.format(size), rule.close);
};

MyEditor.prototype.showLayer = function(layer)
{
	var layer = MyEditor.check(layer);
	if (layer.style.display != "none") return;
	layer.style.display = "";
};

MyEditor.prototype.hideLayer = function(layer)
{
	var layer = MyEditor.check(layer);
