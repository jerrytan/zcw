		...
		if (!Page.IsPostBack)
         {
            ...
            conn.Open();          
		    string gys_id = Request["gys_id"];  //获取首页最受欢迎的供应商传过来的gys_id
            SqlDataAdapter da = new SqlDataAdapter("select 供应商 ,gys_id from 材料供应商信息表 where gys_id='"+gys_id+"'", conn);
			...
			SqlDataAdapter da2 = new SqlDataAdapter("select 品牌名称,pp_id from 品牌字典 where gys_id='"+gys_id+"'", conn);
            DataSet ds2 = new DataSet();
            da2.Fill(ds2, "材料供应商信息表");            
            dt2 = ds2.Tables[0];
			
			//子查询嵌套 先根据传过来的gys_id查品牌名称  再从品牌字典里查复合条件的gys_id 最后根据复合条件的gys_id查分销商信息
			SqlDataAdapter da3 = new SqlDataAdapter("select 供应商,联系人,联系人手机,联系地址 from 材料供应商信息表 where gys_id in(select gys_id from 品牌字典 where 品牌名称 in(select 品牌名称 from 品牌字典 where gys_id='"+gys_id+"') )", conn);
...
<div class="gysxx">
<div class="gysxx1"><a href="index.aspx">首页 ></a>&nbsp&nbsp 
...
<div class="gydl"><div class="dlpp">公司旗下品牌</div>
    <%foreach(System.Data.DataRow row in dt2.Rows){%>
     <div class="gydl1"><img src="images/222_03.jpg" /><%=row["品牌名称"].ToString()%></div>
	 <%}%>   
</div>
...
<div class="gydl"><div class="dlpp">分销商</div>
    <div class="fxs1"><select name="" class="fu1"><option>华北</option></select>  <select name="" class="fu2"><option>北京</option></select>省（市）
    <select name="" class="fu3"><option>石家庄</option></select> 地区<select name="" class="fu4"><option>市区</option></select> 区（县） </div>
