...
<script runat="server">  
        ...
        protected DataTable GetPageList(string cl_id, int begin, int end,string address)
        {
            ...
            //执行分页的sql语句
             string str_sqlpage = @"select 供应商,联系人,联系人手机,联系地址,gys_id from (select ROW_NUMBER() over (order by gys_id) as RowId,* from 材料供应商信息表 where gys_id in(select fxs_id from 分销商和品牌对应关系表 where pp_id = (select pp_id from 材料表 where cl_id= @cl_id )))t where t.RowId between @begin and @end and t.联系地址 like '%'+@address+'%'";
            //添加相应参数值
